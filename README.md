...
Untrusted input is a dependency; it can only ever become one of a fixed set of decisions over a
...
- **Untrusted input:** `fetch<web>` — a dependency
- **Closed type:** `type Decision = AllowDep(Dep) | BlockDep` where `Dep` is closed
- **Sink / capability:** `grant auditDep confidence 70` → `privileged { auditDep(d) }`
- **Consequence axes:** Trust, Confidence

## Run the demo

```sh
examples/apps/dependency-audit/demo.sh
```
...
- `dependency-audit_safe.wave` — the correct design.
- `dependency-audit_unsafe.wave` — the tempting-but-wrong version (the negative example a model must learn to reject).
